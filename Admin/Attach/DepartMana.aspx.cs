...
{
    public partial class DepartMana : System.Web.UI.Page
    {
        ...
        public void BindData()
        {
            ...
            switch(ddlStatus.SelectedItem.Value)
            {
                ...
                case "1":
                    ...
                    break;
                case "2":
                    strWhere.AppendFormat(" Status in('0','1') ");
                    break;
                default:
                    strWhere.AppendFormat(" Status in('0','1') ");
                    break;
            ...
            {
                strWhere.AppendFormat(" and DepartName like '%{0}%' ", txtKeyword.Text.Trim());
            }
            else
            {
        ...
        protected void gridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "MemDelete")
            {
                ...
                depart = bll.GetModel(int.Parse(e.CommandArgument.ToString()));
                depart.Status = "0";
                bll.Update(depart);
                BindData();
            }
            if (e.CommandName == "DepartTop")
            {
                Cultural.Model.Department depart = new Cultural.Model.Department();
                depart = bll.GetModel(int.Parse(e.CommandArgument.ToString()));
                depart.Weight = depart.Weight==9?8:9;
                bll.Update(depart);
                BindData();
            }
        }
